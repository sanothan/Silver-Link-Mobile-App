...
export function RequestForm({
  ...
  const [date, setDate] = useState(
    initial ? dateText(initial.preferredDate) : "",
  );
  const [time, setTime] = useState(initial?.preferredTime ?? "");
  const [durationLabel, setDurationLabel] = useState(
    initial?.durationLabel ?? "1 hour",
  );
  ...
  async function submit() {
    ...
    if (description.length > 500)
      return setError("Please keep the description under 500 characters.");
    await onSubmit({
      activityType,
      description,
  ...
  return (
    <View style={styles.form}>
        ...
        {ACTIVITIES.map((item) => (
          ...
          >
            <Text
              style={[
                styles.chipText,
                activityType === item && styles.chipTextActive,
              ]}
            >
              {item}
            </Text>
      ...
      <Field
        ...
        placeholder="I need help collecting medicine from the nearby pharmacy."
        multiline
      />
      ...
      <View style={styles.chips}>
        {DURATIONS.map((item) => (
          <Pressable
            key={item.label}
            accessibilityRole="radio"
            accessibilityState={{ checked: durationLabel === item.label }}
          ...
          >
            <Text
              style={[
                styles.chipText,
                durationLabel === item.label && styles.chipTextActive,
              ]}
            >
              {item.label}
            </Text>
      ...
      <Field
        ...
        placeholder="Your locality or meeting place"
      />
      {error ? (
...
const styles = StyleSheet.create({
  ...
  buttonText: { color: colors.textOnPrimary, fontSize: 18, fontWeight: "800" },
